...
def _load_local_safetensors(model, safetensors_path):
    """Load CLIP weights from local safetensors file."""
    from safetensors.torch import load_file
    state_dict = load_file(safetensors_path)
    model_state = model.state_dict()
    filtered = {k: v for k, v in state_dict.items() if k in model_state}
    model.load_state_dict(filtered, strict=False)
    print(f"Loaded weights from local safetensors: {safetensors_path}")
    return model
